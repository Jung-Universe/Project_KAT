...
/* Sejong: STV_OLD 유효 여부.
   raw_acc_linear = (STV - STV_OLD)/DT 인데, 첫 스텝에서는 STV_OLD 가 0 (또는
   직전 비행에서 남은 값) 이라 가속도가 속도/DT 만큼 튄다.  DT=0.0025 s 기준
   속도 1 m/s 면 400 m/s^2 이 나와 ACCEL_LIMIT_LINEAR(20) 로 포화되고, LPF 를
   거쳐도 여러 스텝 동안 남는다.  홈 초기화 때 false 로 내려서 첫 스텝의
   가속도를 0 으로 만든다. */
static bool stv_old_valid = false;
...
void CLAW_step(void)
{
    ...
    if (!home_init) {
        if (fabs(Cur_Lat) > 1.0 && fabs(Cur_Lon) > 1.0) {
            ...
            /* Sejong: x/y 스왑 수정.
               아래 Dest_Lat/Dest_Lon 은 x=위도, y=경도 로 읽는데 (198~199줄)
               여기서는 y 에 위도, x 에 경도를 넣고 있었다.  이 대입은 홈 초기화
               스텝에서 TDCN 이 방금 써 넣은 타겟을 덮어쓰므로, 그 한 스텝 동안
               Target_X / Target_Y 가 위경도가 뒤바뀐 값으로 계산돼 큰 스파이크가
               난다.  x=위도, y=경도 로 바로잡는다.
               원본:
                   CLAW_U.Dest_poti_i.y = (real32_T)Cur_Lat;
                   CLAW_U.Dest_poti_i.x = (real32_T)Cur_Lon;
                   CLAW_U.Dest_poti_i.z = -(real32_T)Cur_Alt;                */
            CLAW_U.Dest_poti_i.x = Cur_Lat;
            CLAW_U.Dest_poti_i.y = Cur_Lon;
            CLAW_U.Dest_poti_i.z = -Cur_Alt;
            ...
            /* Sejong: STV_OLD 도 같이 무효화한다.  STV[0..2] 는 아래 258줄쯤에서
               계산되므로 여기서 대입하면 직전 스텝 값이 들어간다.  플래그만
               내리고 실제 대입은 가속도 계산 직전에 한다. */
            stv_old_valid = false;
    ...
    if (home_init) {
        /* Sejong: Lon2m() 인자 수정.  Lon2m 은 "그 위도에서 경도 1도가 몇 m 인가"
           를 주는 함수라 인자가 위도여야 한다.  경도를 넣으면 cos(경도) 를 쓰게
           돼 값이 완전히 달라지고, 한국 경도(127도) 에서는 부호까지 뒤집힌다
           (배율 -0.76).  SITL state 6 로그에서 실제로 동쪽 +2.80 m 를 CLAW 가
           -2.92 m 로 계산하는 것을 확인했다.
           원본:  STV[10] = (Cur_Lon - Home_Lon) * Lon2m(Home_Lon);           */
    ...
    /* Sejong: TDCN 이 CLAW_step() 호출 전에 XTV[0..2] 에 EKF 속도를 채운다.
       원본은 아래 3줄로 0 을 대입해 속도 피드백을 무효화하고 있었다.
       프레임은 정규 NED, 단위 m/s: XTV[0]=North, XTV[1]=East, XTV[2]=Down.
       (TDCN 은 고도만 up 으로 주고 CLAW 가 Cur_Alt = -Cur_Pos.z 로 뒤집으므로
        STV[11] / Target_Z / z1[0] / z1_dot[0] / z2[0] 이 모두 down-positive 다)
       (새 CLAW 버전을 받으면 이 3줄을 다시 제거해야 한다)
    XTV[0] = 0;
    XTV[1] = 0;
    XTV[2] = 0;                                                             */
...
#define ACCEL_LIMIT_ANGULAR 10.0
    ...
    /* Sejong: 첫 스텝은 이전 속도가 없으므로 가속도를 0 으로 둔다.
       (STV_OLD 를 현재 속도로 채우면 raw_acc_linear 가 0 이 된다) */
    if (!stv_old_valid) {
        STV_OLD[0] = STV[0];
        STV_OLD[1] = STV[1];
        STV_OLD[2] = STV[2];
    ...
    if (home_init) {
        /* Sejong: 위 STV[10] 과 같은 Lon2m() 인자 수정.
           원본:  Target_Y = (Dest_Lon - Home_Lon) * Lon2m(Home_Lon);         */
        Target_X = (Dest_Lat - Home_Lat) * Lat2m(Home_Lat);
        Target_Y = (Dest_Lon - Home_Lon) * Lon2m(Home_Lat);
    ...
    /* Sejong: Dest_poti 가 double 이 되어 real32_T 캐스팅을 뺐다.
       원본:
           CLAW_Y.cur_poti.x = (real32_T)STV[9];
           CLAW_Y.cur_poti.y = (real32_T)STV[10];
           CLAW_Y.cur_poti.z = -(real32_T)STV[11];                          */
    CLAW_Y.cur_poti.x = STV[9];
    CLAW_Y.cur_poti.y = STV[10];
    CLAW_Y.cur_poti.z = -STV[11];
}
